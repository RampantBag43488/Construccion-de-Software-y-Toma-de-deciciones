...
const CAR_WIDTH = 32;
const CAR_HEIGHT = 56;
//const ROAD_SPEED = 500; // píxeles por segundo

// Cambiar road speed por:
let roadSpeed = 500;
const BASE_ROAD_SPEED = 500;
const BASE_SPAWN_INTERVAL = 0.9; // segundos entre spawns
...
function update(dt) {
  ...
  roadSpeed = BASE_ROAD_SPEED + Math.min(score/4, 500); // Aumentar velocidad con el puntaje
...
spawnTimer -= dt;
  ...
  if (spawnTimer <= 0) {
    spawnTimer = Math.max(0.30, BASE_SPAWN_INTERVAL - score/5000); // Reducir intervalo con el puntaje, mínimo 0.30s
